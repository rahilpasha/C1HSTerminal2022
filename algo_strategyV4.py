...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def da_strat(self, game_state):  # version 4
        """
    ...
    def funnel(self, game_state):
        ...
        if [13, 0] == self.least_damage_spawn_location(game_state, [[13, 0], [14, 0]]):
            ...
            min_right_walls = [[0, 13], [1, 13], [3, 10], [20, 10], [21, 10], [22, 10], [4, 9], [5, 8], [18, 8], [6, 7], [7, 7], [8, 7], [9, 7]]
            min_right_turrets = [[2, 11], [23, 11], [19, 9]]
            game_state.attempt_spawn(WALL, min_right_walls)
            game_state.attempt_spawn(TURRET, min_right_turrets)
            ...
            # place the secondary walls and turrets for a funnel
            right_turrets = [[1, 12], [2, 12], [22, 12], [23, 12], [20, 9], [7, 6], [16, 6], [17, 6]]
            right_walls = [[22, 13], [23, 13], [19, 10], [19, 8]]
            ...
            game_state.attempt_upgrade([[13, 3], [14, 3]])

            # remove the non-static walls and turrets
            game_state.attempt_remove(right_walls)
            game_state.attempt_remove(min_right_walls)
            game_state.attempt_remove(right_turrets)
            game_state.attempt_remove(min_right_turrets)

            if game_state.get_resource(1, 0) >= 11 and game_state.turn_number % 3 == 1:
                # attack strat

                # check if we have at least  17 mobility points
                if game_state.turn_number > 25 and game_state.get_resource(1, 0) >= 30:

                    # split into 4 squads: interceptors at the front and two squads of scouts at the back and two demolishers that diverge from the group
                    squad = int(game_state.get_resource(1, 0) // 2)
                    game_state.attempt_spawn(INTERCEPTOR, [26, 12], 5)
                    game_state.attempt_spawn(SCOUT, [15, 1], squad - 6)
                    game_state.attempt_spawn(DEMOLISHER, [19, 5], 2)
                    game_state.attempt_spawn(SCOUT, [13, 0], 100)

                elif game_state.get_resource(1, 0) >= 17:

                    # split into two squads of scouts that are staggered at the back
                    squad = int(game_state.get_resource(1, 0) // 2)
                    game_state.attempt_spawn(SCOUT, [15, 1], squad - 2)
                    game_state.attempt_spawn(SCOUT, [13, 0], 100)
            ...
            # place the minimum required walls and turrets for a funnel
            min_left_walls = [[26, 13], [27, 13], [5, 10], [6, 10], [7, 10], [24, 10], [23, 9], [9, 8], [22, 8], [18, 7], [19, 7], [20, 7], [21, 7]]
            min_left_turrets = [[4, 11], [25, 11], [8, 9]]
            game_state.attempt_spawn(WALL, min_left_walls)
            game_state.attempt_spawn(TURRET, min_left_turrets)
            ...
            # place the secondary walls and turrets for a funnel
            left_turrets = [[4, 12], [5, 12], [25, 12], [26, 12], [9, 9], [10, 6], [11, 6], [20, 6]]
            left_walls = [[4, 13], [5, 13], [8, 10], [8, 8]]
            ...
            game_state.attempt_remove(min_left_turrets)
            ...
            if game_state.get_resource(1, 0) >= 11 and game_state.turn_number % 3 == 1:
                ...
                if game_state.turn_number > 25 and game_state.get_resource(1, 0) >= 30:
                    ...
                    # split into 4 squads: interceptors at the front and two squads of scouts at the back and two demolishers that diverge from the group
                    squad = int(game_state.get_resource(1, 0) // 2)
                    game_state.attempt_spawn(INTERCEPTOR, [1, 12], 5)
                    game_state.attempt_spawn(SCOUT, [12, 1], squad - 6)
                    game_state.attempt_spawn(DEMOLISHER, [8, 5], 2)
                    ...
                    # split into two squads of scouts that are staggered at the back
                    squad = int(game_state.get_resource(1, 0) // 2)
                    game_state.attempt_spawn(SCOUT, [12, 1], squad - 2)
                    game_state.attempt_spawn(SCOUT, [14, 0], 100)

    def supports(self, game_state):
        # spawn extra supports
        support_locations = [[12, 4], [13, 4], [14, 4], [15, 4], [10, 5], [11, 5], [12, 5], [13, 5], [14, 5], [15, 5], [16, 5], [17, 5]]
